...
API_HASH = environ.get('API_HASH', '0123456789abcdef0123456789abcdef')
BOT_TOKEN = environ.get('BOT_TOKEN', "7012772836:AAFBwKBA5S3s7ZmLTmWlMdf_U1NgC85z_vk")
...
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1001722859321').split()]
auth_users = [int(user) if id_pattern.search(user) else user for user in environ.get('AUTH_USERS', '').split()]
AUTH_USERS = (auth_users + ADMINS) if auth_users else []
PREMIUM_USER = [int(user) if id_pattern.search(user) else user for user in environ.get('PREMIUM_USER', '6287591671').split()]
auth_channel = environ.get('AUTH_CHANNEL', '')
auth_grp = environ.get('AUTH_GROUP')
AUTH_CHANNEL = int(auth_channel) if auth_channel and id_pattern.search(auth_channel) else None
AUTH_GROUPS = [int(ch) for ch in auth_grp.split()] if auth_grp else None
support_chat_id = environ.get('SUPPORT_CHAT_ID', '-1002088051309')
reqst_channel = environ.get('REQST_CHANNEL_ID', '-1001722859321')
REQST_CHANNEL = int(reqst_channel) if reqst_channel and id_pattern.search(reqst_channel) else None
SUPPORT_CHAT_ID = int(support_chat_id) if support_chat_id and id_pattern.search(support_chat_id) else None
NO_RESULTS_MSG = bool(environ.get("NO_RESULTS_MSG", True))

# MongoDB information
DATABASE_URI = environ.get('DATABASE_URI', "mongodb://[rani828719:sVyRWZOrUzIWNfHp@cluster0]host1[:port1][,...hostN[:portN]][/[defaultauthdb]?retryWrites=true&w=majority")
...
CHNL_LNK = environ.get('CHNL_LNK', 'https://t.me/kung_fu_panda_4_movie_hindi_dub')
